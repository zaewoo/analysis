...
- 이들 범주는 범주의 범위에서 차이가 있고 데이터의 수에서도 차이가 있으므로 그 정확도가 낮게 나왔을 것으로 생각습니다. 
- 모든 특성에서 영화 관객 수의 가중치를 특성의 영화 관객 수의 평균값, 혹은 중앙값을 사용하였으므로 대체로 보수적인 예측을 하였을 것으로 생각하였습니다. 그러므로 영화 관객 수의 규모가 클수록 제대로 된 예측이 어려웠을 것으로 생각하였습니다. 
...
영화 장르가 액션, 그리고 범죄에 해당하는 영화는 학습 모델의 정확도가 상대적으로 낮습니다. 반면 영화 장르가 다큐멘터리에 해당하는 영화는 학습 모델의 정확도가 상대적으로 높습니다. 그 이유에 대해서는 다음과 같이 생각해 보았습니다.

- 영화 장르의 특성 상 영화 관객 수의 분포가 골고루 분포해 있는 영화, 혹은 영화 관객 수의 분포가 밀집되어 있는 영화가 있고, 그 데이터를 학습하였으므로 학습 모델의 정확도에서 차이가 있을 것이라고 생각했습니다.
- 예를 들어 **안소니 루소** 감독의 영화 **어벤져스: 엔드게임**은 그 장르가 **액션**입니다. 그리고 이 영화의 관객 수는 약 **1,400만 명**입니다. **최동훈** 감독의 영화 **암살**은 그 장르가 **액션**입니다. 그리고 이 영화의 관객 수는 약 **1,300만 명**입니다. 반면 **다니엘 에스피노사** 감독의 영화 **모비우스**는 그 장르가 **액션**입니다. 그리고 이 영화의 관객 수는 약 **50만 명**입니다. 이처럼 영화의 장르가 **액션**인 영화는 영화의 장르 특성 상 영화 관객 수의 분포가 골고루 분포해 있습니다. 
- 반면 **진모영** 감독의 영화 **님아, 그 강을 건너지 마오**는 그 장르가 **다큐멘터리**입니다. 그리고 이 영화의 관객 수는 약 **480만 명**입니다. 이는 영화의 장르가 **다큐멘터리**인 영화 중에서 가장 많은 영화 관객을 동원하였습니다. **이충렬** 감독의 영화 **워낭소리**는 그 장르가 **다큐멘터리**입니다. 그리고 이 영화의 관객 수는 약 **300만 명**입니다. 이는 **님아, 그 강을 건너지 마오**를 이어 두 번째로 가장 많은 영화 관객을 동원하였습니다. 이처럼 영화의 장르가 **다큐멘터리**인 영화는 영화의 장르 특성 상 영화 관객 수의 분포가 상대적으로 밀집되어 있습니다.
- 학습 모델의 정확도는 위의 이유에서 기인하였다고 생각하였습니다.
