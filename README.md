...
### 데이터: 전처리
OpenAPI로 불러온 데이터는 관객수 정보가 없으므로 KOFIC에서 크롤링한 정보와 JOIN을 해주어야 한다

KOBIS API 테이블의 movieNm, openDt를 기준으로 KOFIC 크롤링 데이터 테이블의 movieNm 과 openDt 일치하는 경우에 누적 관객수를 join한다

- JOIN 방식은 Google Cloud에서 제공하는 SQL툴을 사용하여 JOIN후, 테이블을 csv파일로 저장하였다

JOIN 테이블은 총 14,778개의 데이터로 이루어져있다.
