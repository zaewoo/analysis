...
- 종합 비교 분석 표에서는 세번째부터 다섯번째 시도만 포함 되어 있다. 가장 안정적이고 비교가 가능한 성능 차이라 고려되어 세 시도만 포함하였다.
- 모델은 GradientBoost 혹은 XGBoost가 Test Accuracy도 높은 편이고 Train Accuracy와의 차이도 적은 편이라 가장 적합한 모델로 판단된다.
- GridSearchCV 또한 성능이 좋게 나왔지만, GradientBoost 또는 GridSearchCV에 비해 비교적 많은 시간이 필요하기에 고려하지 않았다.
- Trial 3의 성능이 가장 높게 나왔기에 새로운 데이터로 prediction을 해보기로 한다.
