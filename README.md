...
#### 영화 장르
이 컬럼은 해당 영화의 장르를 나타내는 컬럼입니다. 이 컬럼에서는 범주, 즉 영화의 장르를 간소화하였습니다. 전체 데이터는 여러 가지 영화 장르가 포함되어 있습니다. 
...
#### 영화 배급·수입사 · 영화 감독 · 영화 배우
이 컬럼은 해당 영화의 영화 배급·수입사, 영화 감독, 그리고 영화 배우를 나타내는 컬럼입니다. 이 컬럼에서는 필요하지 않은 컬럼을 제거하였습니다. 그 방법은 아래의 표와 함께 설명하겠습니다.

<div align="Center">
    
|인덱스|영화 제목|영화 배우|영화 배우|영화 배우|
|:--:|--|--|--|--|
...
#### 영화 관객 수
이 컬럼은 해당 영화의 관객 수를 나타내는 컬럼입니다. 이 컬럼에서는 영화의 관객 수가 1,000명이 되지 않는 영화는 모두 제거하였습니다. 그리고 영화의 관객 수를 카테고리 별로 분류하였습니다.
...
#### 영화 러닝 타임
이 컬럼은 해당 영화의 러닝 타임을 나타내는 컬럼입니다. 이 컬럼에서는 영화의 러닝 타임이 제공되지 않는 영화는 모두 제거하였습니다. 

#### 영화 배급·수입사
이 컬럼은 영화의 제작에 관여한 배급·수입사를 나타내는 컬럼입니다. 이 컬럼에서는 영화의 제작에 관여한 배급·수입사가 제공되지 않는 영화는 모두 제거하였습니다. 
    
#### 영화 감독
이 컬럼은 영화의 제작에 관여한 영화 감독을 나타내는 컬럼입니다. 이 컬럼에서는 영화의 제작에 관여한 영화 감독이 제공되지 않는 영화는 모두 제거하였습니다. 
    
#### 영화 배우
이 컬럼은 영화의 제작에 관여한 영화 배우를 나타내는 컬럼입니다. 이 컬럼에서는 영화의 제작에 관여한 영화 배우가 제공되지 않는 영화는 [네이버 영화](https://movie.naver.com), [다음 영화](https://movie.daum.net/main), [왓챠피디아](https://pedia.watcha.com/ko-KR/), [IMDB](https://imdb.com), [Rotten Tomatoes](https://www.rottentomatoes.com) 등에서 확인하고 추가하였습니다. 특히 애니메이션 장르의 영화는 영화 배우·성우 등이 포함되지 않은 경우가 많았기 때문에 이 역시 위의 사이트를 확인하고 추가하였습니다. 

#### 국가
이 컬럼은 영화를 제작한 국가를 나타내는 컬럼입니다. 이 컬럼에서는 범주, 즉 영화 제작 국가를 간소화하였습니다. 전체 데이터는 여러 국가가 포함되어 있습니다. 그 방법은 영화를 제작한 국가를 크게 **한국**, **미국**, **그 외 국가**로 분류하였습니다. 그 이유는 다음과 같습니다. 영화진흥위원회에 발간한 **한국영화연감**에 의하면 국내에서 상영된 영화 중에서 한국 영화의 관객 점유율은 51.4%로 가장 높았고, 미국 영화의 관객 점유율은 41.1%를 기록했습니다. 이 둘을 합한 점유율은 92.5%로 국내 대다수를 차지하고 있습니다. 그 외 국가에서 개봉된 영화는 영화 관객 점유율이 현저히 낮습니다. 그러므로 **한국**, **미국**, 그리고 **그 외 국가**로 분류하였습니다. 이는 가장 최신의 자료 2018년에 발간된 자료를 기준으로 하였습니다. 
...
#### 영상물 심의 등급
이 컬럼은 영상물 심의 등급을 나타내는 컬럼입니다. 이 컬럼에서는 영상물 등급 위원회에서 정한 바에 따라 청소년 관람불가, 그 외로 분류하였습니다. 

#### 영화 개봉 일자
이 컬럼은 영화 개봉 일자를 나타내는 컬럼입니다. 이 컬럼에서는 별도의 전처리가 필요하지 않았습니다. 그 밖에 구체적인 데이터 분석을 위해서 이 데이터를 활용하여 **월 단위**의 컬럼을 새로이 만들었습니다.

## 본론: 모델

### 모델: 개요
학습 모델의 성능은 총 다섯 번의 시도로 개선하였습니다. 다섯 번의 시도 끝에 성능이 가장 좋은 모델은 **(1)Decision Tree Classifier, (2)XGB Classifier**이고, 모델의 성능은 약 74%의 정확도를 갖습니다. 
